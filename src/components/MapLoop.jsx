...
export default function Maps2() {
  ...
  useEffect(() => {
    ...
    map.current = new maptilersdk.Map({
      ...
      zoom: zoom,
      language: "en",
      // Remove controls and attribution
      attributionControl: false, // This removes the copyright
      logoPosition: "bottom-left", // You can change position or remove via CSS
      // Optional: Remove other controls
      navigationControl: false, // Remove zoom buttons if you want
      geolocateControl: false, // Remove geolocation button if present
      interactive: false,
      boxZoom: false,
  ...
  const addMarkers = () => {
    ...
    new maptilersdk.Marker({
      ...
      offset: [0, -15], // Center the bottom of the icon at the coordinate
    })
  ...
  // Updated delivery person simulation with bike icon
  // Continuous looping without reset delay
  const startRealPathSimulation = (coordinates) => {
    let currentIndex = 0;
    let bikeIcon;
    let deliveryPersonMarker;

    const createBikeMarker = () => {
      bikeIcon = createCustomIconElement(_bikeIcon, 60, "", true);
      deliveryPersonMarker = new maptilersdk.Marker({
        element: bikeIcon,
        offset: [0, -15],
      })
